...
<div class="container">
    ...
    <form action="{{ route('etudiants.enregistrer') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <select name="filiere_id" id="filiere_id" required>
                ...
                @foreach ($filieres as $filiere)
                    <option value="{{ $filiere->id }}" @if(old('filiere_id') == $filiere->id) selected @endif>
                        {{ $filiere->libelle }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="form-group">
            ...
            <select name="niveau_id" id="niveau_id" required>
                ...
                @foreach ($niveaux as $niveau)
                    <option value="{{ $niveau->id }}" @if(old('niveau_id') == $niveau->id) selected @endif>
                        {{ $niveau->libelle }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="form-group">
            ...
            <select name="annee_id" id="annee_id" required>
                ...
                @foreach ($annees_academiques as $annee)
                    <option value="{{ $annee->id }}" @if(old('annee_id') == $annee->id) selected @endif>
                        {{ $annee->libelle }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="form-group">
            <label for="validite_carte">Durée de validité de la carte (en mois)</label>
            <input type="number" name="validite_carte" id="validite_carte" value="{{ old('duree_validite') }}" min="1" required>
        </div>
